...
if (process.env.NODE_ENV !== 'test') {
  if (!process.env.NEXTAUTH_URL) {
    throw new Error(
      'FATAL: NEXTAUTH_URL environment variable is not set.\n' +
      'NextAuth requires NEXTAUTH_URL to function properly.\n' +
      'Set it to your application URL (e.g., https://yourapp.railway.app or http://localhost:3000)'
    );
  }
  ...
  if (!process.env.NEXTAUTH_SECRET) {
    throw new Error(
      ...
      'NextAuth requires NEXTAUTH_SECRET for session encryption.\n' +
  ...
  if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
    throw new Error(
      ...
      'These are required for Google OAuth authentication.'
    );
  }
}
