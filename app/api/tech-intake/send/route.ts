              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0;padding:20px 24px;background:#f8fafb;border-radius:8px;border:1px solid #e5e7eb;">
                <tr>
                  <td>
                    ...
                    <p style="margin:0;font-size:13px;color:#555;">&#10003; Any upcoming travel dates around launch</p>
                  </td>
                </tr>
              </table>

              <p style="margin:24px 0 0;font-size:12px;color:#999;">This link is unique to you and expires on ${expiresDate}. The form takes about 3–5 minutes.</p>
...
The email asks them to complete their Technology Intake Form — a quick form to capture their hardware, shipping, and IT preferences so Farther can ship their equipment on time.

Rules:
- Start with "Hi ${advisorFirstName}," (greeting is required)
- Keep it to 2-3 short paragraphs maximum — this is a quick, easy form
- Warm, excited tone — they're getting their new tech setup!
- Mention this takes about 3-5 minutes and helps us get equipment shipped before their start date
...
- End the last paragraph naturally — no "Best regards" or sign-off needed`,
      ...
      console.error('[tech-intake send] AiZolo email generation failed, using fallback:', aiErr);
      emailBodyText = '';
    }
...
We're getting closer to your launch day and it's time to get your technology set up! We have a quick form for you to fill out so we can configure and ship your equipment on time.

It only takes about 3–5 minutes — just your laptop preference, shipping address, and a few IT details. The sooner we have this back, the sooner your gear is on its way.`;
    }
    ...
    const emailRes = await fetch('https://api.hubapi.com/crm/v3/objects/emails', {
      ...
      body: JSON.stringify({
        properties: {
          hs_email_subject: `Your Technology Intake Form — Let's Get Your Gear Shipped!`,
    ...
    if (emailRes.ok) {
      ...
      console.error('[tech-intake send] HubSpot email failed:', await emailRes.text());
    }
    ...
    console.error('[tech-intake send]', err);
